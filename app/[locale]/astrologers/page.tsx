...
export default function AstrologersPage() {
  ...
  const handleStart = async (aiAstrologerId: number) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || ta('chatStartFailed'));
    }
  };
  ...
  const handleCreateKundli = async () => {
    ...
    try {
      ...
      toast.error(error?.data?.message || ta('kundliCreateFailed'));
      return false;
    }
  };
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-16">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-gray-500 font-medium">{ta('loadingList')}</div>
            </div>
          </div>
        ) : (
                ...
                <CardHeader className="relative pb-2 pt-14">
                  <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2">
                    <div className="relative">
                      ...
                      <span
                        ...
                        title={item.isOnlineNow ? ta('onlineNow') : ta('offline')}
                      />
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-[#7b4c34] mt-0.5 truncate">
                      {item.persona || ta('personaFallback')}
                    </p>
                ...
                <CardContent className="flex h-full flex-col space-y-3">
                  ...
                  <div className="rounded-xl border border-orange-100 bg-orange-50/40 p-2.5">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[#7b4c34]">{ta('consultationPrice')}</span>
                      <span className="inline-flex items-center gap-1 text-sm font-semibold text-[#2f1d12]">
                        <Sparkles className="h-4 w-4 text-primary" />
                        {ta('coinsPerMin', { n: item.coinsPerMinute })}
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    <span className="inline-flex items-center gap-1.5">
                      {item.isOnlineNow === false ? ta('currentlyOffline') : ta('startChat')}
