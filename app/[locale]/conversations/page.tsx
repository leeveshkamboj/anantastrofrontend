                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <Image
                      ...
                      alt={s.aiAstrologer?.displayName || t('astrologerFallback')}
                      width={30}
                      height={30}
                      className="rounded-full border bg-white"
                    />
                    <p className="truncate font-semibold text-gray-900">
                      {s.aiAstrologer?.displayName || t('sessionNumber', { id: s.id })}
                    </p>
                  </div>
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full border bg-gray-50 px-2 py-1">
                      ...
                      {t('coinsDebitedLine', { n: s.totalCoinsDebited ?? 0 })}
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-between gap-3 sm:justify-end">
                  ...
                  >
                    {String(s.status ?? '').toLowerCase() === 'active'
                      ? t('statusActive')
                      : String(s.status ?? '').toLowerCase() === 'ended'
                        ? t('statusEnded')
                        : String(s.status ?? '')}
                  </Badge>
