...
function toForm(row: ChatAstrologer): AdminAstrologerForm {
  return {
    ...
    coinsPerMinute: String(row.coinsPerMinute ?? 20),
    gender: row.gender ?? '',
...
export default function AdminAiAstrologersPage() {
  ...
  const create = async () => {
    try {
      ...
      toast.error(error?.data?.message || t('aiCreateFailed'));
    }
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      toast.error(error?.data?.message || t('aiUpdateFailed'));
    }
  };
  ...
  const remove = async (id: number) => {
    try {
      ...
      refetch();
      toast.success(t('aiDeleted'));
    } catch (error: any) {
      toast.error(error?.data?.message || t('aiDeleteFailed'));
    }
  };
