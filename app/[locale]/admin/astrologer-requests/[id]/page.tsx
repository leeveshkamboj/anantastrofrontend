...
export default function AdminAstrologerRequestDetailPage() {
  ...
  const handleApprove = async () => {
    try {
      ...
      toast.error(error?.data?.message || t('requestApproveFailed'));
    }
  };

  const handleReject = async () => {
    if (!rejectionNote.trim()) {
      toast.error(t('rejectionNote'));
      return;
    }
    try {
      ...
      toast.error(error?.data?.message || t('requestRejectFailed'));
    }
  };
  ...
  const handleAllowReapply = async () => {
    try {
      ...
      toast.success(t('canReapply'));
      refetch();
    } catch (error: any) {
      toast.error(error?.data?.message || t('reapplyFailed'));
    }
  };
