...
export default function AstrologerProfilePage() {
  ...
  const handleSaveDraft = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || t('saveDraftFailed'));
    }
  };
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadResult?.isSuccess && uploadResult?.data) {
        ...
        toast.success(t('imageUploaded'), { id: 'upload-image' });
      } else {
        // Reset preview and file on failure
        setProfileImagePreview(null);
        setProfileImageFile(null);
        toast.error(t('imageUploadFailed'), { id: 'upload-image' });
      }
    } catch (error: any) {
      // Reset preview and file on error
      setProfileImagePreview(null);
      setProfileImageFile(null);
      toast.error(error?.data?.message || t('imageUploadFailed'), { id: 'upload-image' });
    }
  ...
  const handleSubmitForReview = async (e: React.FormEvent) => {
    ...
    if (missing.size > 0) {
      ...
      toast.error(t('missingFields', { fields: missingList }));
    ...
    try {
      ...
      // Clear the uploaded UUID ref after successful save
      uploadedImageUuidRef.current = null;
      toast.success(t('submittedReview'));
    } catch (error: any) {
      const errorMessage = error?.data?.message || t('submitReviewFailed');
  ...
  const handleUserSettingsUpdate = async () => {
    try {
      ...
      toast.error(error?.data?.message || t('settingsUpdateFailed'));
    }
  };
