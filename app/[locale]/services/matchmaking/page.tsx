...
export default function MatchmakingPage() {
  ...
  const resolveLatLngTz = async (p: PartnerForm): Promise<{ lat: number; lng: number; tz: number }> => {
    ...
    if (p.latitude != null && p.longitude != null) {
      ...
      if (d?.lat != null && d?.lng != null) {
        ...
        throw new Error(te('locationNotFoundSelect'));
      }
    }
    let tz = 5.5;
  ...
  const validatePartner1 = (): { valid: boolean; message?: string } => {
    ...
    if (!partner1.name?.trim()) return { valid: false, message: tv('p1Name') };
    if (!partner1.dob?.trim()) return { valid: false, message: tv('p1Dob') };
    if (!partner1.time?.trim()) return { valid: false, message: tv('p1Time') };
    if (!partner1.placeOfBirth?.trim()) return { valid: false, message: tv('p1Place') };
    return { valid: true };
  };

  const validatePartner2 = (): { valid: boolean; message?: string } => {
    if (useProfile2 != null) return { valid: true };
    if (!partner2.name?.trim()) return { valid: false, message: tv('p2Name') };
    if (!partner2.dob?.trim()) return { valid: false, message: tv('p2Dob') };
    if (!partner2.time?.trim()) return { valid: false, message: tv('p2Time') };
    if (!partner2.placeOfBirth?.trim()) return { valid: false, message: tv('p2Place') };
    return { valid: true };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const getPartnerPayload = async (which: 1 | 2) => {
      ...
      if (!dob || !time?.trim()) throw new Error(te('partnerDobTime', { which }));
      if (!form.placeOfBirth?.trim() && (form.latitude == null || form.longitude == null) && !prof) {
        throw new Error(te('partnerPlaceCoords', { which }));
      }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : te('failedMatchmaking');
      toast.error(message);
    }
  };
