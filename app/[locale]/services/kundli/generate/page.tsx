...
function KundliGenerateContent() {
  ...
  const startGenerationAndRedirect = async (
    ...
    if (!place?.trim()) {
      toast.error(te('placeRequiredChart'));
      return;
    }
    ...
    if (existingLatLng?.latitude != null && existingLatLng?.longitude != null) {
      ...
      try {
        ...
        if (data?.lat != null && data?.lng != null) {
          ...
          if (!fallback) {
            toast.error(te('locationNotFoundSelect'));
            return;
          }
          lat = fallback.lat;
          lng = fallback.lon;
    ...
    try {
      ...
      if (err.status === 402 || err.code === 'INSUFFICIENT_COINS') {
        toast.error(err.message ?? te('insufficientCoins'));
        router.push('/pricing');
        return;
      }
      if (err.status === 409 || err.code === 'DUPLICATE_CHART_DETAILS') {
        toast.error(
          err.message ?? te('duplicateChart'),
        );
        return;
      }
      toast.error(te('failedKundliGen'));
    }
  };
    ...
    try {
      if (editingProfileId != null) {
        ...
        if (result?.data?.id) {
          ...
          setPlaceOfBirth('');
          refetch();
          toast.success(te('profileUpdated'));
        }
      } else {
...
export default function KundliGeneratePage() {
  return (
    <Suspense fallback={<KundliSuspenseFallback />}>
