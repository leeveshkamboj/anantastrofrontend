...
export default function ChatSessionPage() {
  ...
  const astroName = astrologer?.displayName || tc('astrologerFallback');
  const astroAvatar =
        ...
        <CardHeader className="border-b bg-linear-to-r from-violet-50 via-fuchsia-50 to-indigo-50">
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:pt-4">
            ...
            >
              ...
              {isSessionClosed ? tc('chatEnded') : ending ? tc('ending') : tc('endChat')}
            </Button>
          </div>
          <div className="mt-2 grid grid-cols-1 gap-2 text-xs sm:mt-3 sm:flex sm:flex-wrap sm:items-center">
            <span className="rounded-full border bg-white px-2.5 py-1 text-slate-700">{tc('session')} {elapsedLabel}</span>
            <span className="rounded-full border bg-white px-2.5 py-1 text-slate-700">{tc('nextBill')} {nextBillLabel}</span>
            <span className="rounded-full border bg-white px-2.5 py-1 text-slate-700">{tc('billed')} {billedSecondsLive}s</span>
          </div>
        </CardHeader>
        <CardContent className="space-y-3 p-2.5 sm:p-3 md:space-y-4 md:p-5">
          {lowCoinsForNextMinute && (
            <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-950">{tc('lowBalance')}</p>
          )}
          <div
          ...
          >
            {isSessionClosed && (
              <p className="mb-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">{tc('closedReadOnly')}</p>
            )}
            {isLoading ? (
              <div className="space-y-3 animate-pulse">
                <div className="mr-auto h-10 w-[72%] rounded-2xl border bg-white/80" />
              ...
              <p className="rounded-xl border border-dashed bg-white px-3 py-2 text-sm text-slate-500">{tc('emptyPrompt')}</p>
            ) : (
              messages.map((m) => (
                <div
                  key={m.id}
                ...
                >
                  ...
                  >
                    <p className="whitespace-pre-wrap leading-relaxed">
                      {m.text ||
                        (m.contentType === 'image' ? tc('imageSent') : m.contentType === 'video' ? tc('videoSent') : '')}
                    </p>
                    <div
          ...
          <div className="sticky bottom-0 flex items-center gap-2 rounded-xl border bg-white p-2 shadow-sm">
            <Input
              ...
              placeholder={isSessionClosed ? tc('placeholderClosed') : tc('placeholderType')}
            ...
            >
              {isSessionClosed ? tc('closed') : isLoading ? tc('loading') : sending ? tc('sending') : tc('send')}
            </Button>
          </div>
          <p className="text-center text-xs text-gray-500">
            {isSessionClosed ? tc('billingStopped') : tc('billingActive')}
          </p>
        </CardContent>
      </Card>
    </div>
