    ...
    try {
      ...
      if (result?.data) {
        ...
        if (result.data.user.role === 'admin') {
          toast.success(t('loginToast.success'));
          router.push('/admin');
          return;
        }
      ...
      if (errBody?.code === 'EMAIL_NOT_VERIFIED') {
        ...
        toast.error(t('loginToast.verifyEmail'));
        return;
      }
      const errorMessage = error || errBody?.message || t('loginToast.failed');
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (result.data.alreadyVerified) {
        setEmailNotVerified(null);
        toast.success(t('loginToast.alreadyVerified'));
        return;
      }
      toast.success(t('loginToast.resendSuccess'));
    } catch {
      toast.error(t('loginToast.resendFail'));
    }
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {emailNotVerified && (
              <div className="mt-4 p-3 rounded-md bg-amber-50 border border-amber-200">
                <p className="text-sm text-amber-800 mb-2">{t('login.verifyBanner')}</p>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
