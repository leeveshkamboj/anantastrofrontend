  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      if (fe.status === 409 || fe.code === 'DUPLICATE_CHART_DETAILS') {
        toast.error(fe.message ?? t('registerToast.duplicateChart'));
        return;
      }
      const errorMessage =
        fe.message ||
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (result.data.alreadyVerified) {
        ...
        toast.success(t('registerToast.alreadyVerified'));
        return;
      }
      toast.success(t('registerToast.resendSuccess'));
    } catch {
      toast.error(t('registerToast.resendFail'));
    }
  };
          ...
          {pendingVerifyEmail ? (
            <div className="space-y-4">
              <p className="text-sm text-gray-600 text-center">
                {t('register.pendingVerifyBefore')} <strong>{pendingVerifyEmail}</strong>. {t('register.pendingVerifyAfter')}
              </p>
              <Button
                type="button"
                variant="outline"
              ...
              >
                {isResending ? t('login.sending') : t('login.resendVerification')}
              </Button>
            </div>
          ) : (
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white px-2 text-gray-500">{t('register.orContinueWith')}</span>
              </div>
            </div>
