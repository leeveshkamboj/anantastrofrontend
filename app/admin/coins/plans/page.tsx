...
export default function AdminCoinPlansPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!Number.isFinite(coinQuantity) || coinQuantity < 1) {
      toast.error('Coin quantity must be at least 1');
      return;
    }
    if (!Number.isFinite(pricePaise) || pricePaise < 1) {
      toast.error('Price (paise) must be at least 1');
      return;
    }
    ...
    if (originalRaw !== '' && (!Number.isFinite(originalPricePaise) || (originalPricePaise ?? 0) < 0)) {
      toast.error('Original price must be a valid paise amount');
      return;
    }
    ...
    try {
      ...
      toast.error(editing ? 'Could not update pack' : 'Could not create pack');
    }
  };
