...
import { MessageCircleMore, Sparkles, Users, Zap } from "lucide-react"
...
type ConversationsHeroSectionProps = {
  sessionCount?: number
  activeCount?: number
  totalCoins?: number
}

export function ConversationsHeroSection({
  sessionCount = 0,
  activeCount = 0,
  totalCoins = 0,
}: ConversationsHeroSectionProps) {
          ...
          <div className="flex flex-col items-center gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:gap-6 lg:p-10 xl:gap-10">
            <div className="w-full text-center lg:flex-1 lg:text-left">
              <div className="mb-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-center lg:justify-start">
                ...
                <span className="flex h-11 w-11 items-center justify-center rounded-full border-[3px] border-astro-purple/20 bg-astro-yellow/30 shadow-sm">
                  <MessageCircleMore className="h-5 w-5 text-astro-orange" aria-hidden="true" />
                </span>
              </div>
              ...
              <div className="mb-5 flex flex-wrap items-center justify-center gap-2.5 lg:justify-start">
                ...
                {activeCount > 0 ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3.5 py-2 text-xs font-bold text-emerald-800 ring-1 ring-emerald-600/20">
                    ...
                    {t("heroStatActive", { n: activeCount })}
                  </span>
                ) : null}
                {totalCoins > 0 ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-3.5 py-2 text-xs font-bold text-amber-900 ring-1 ring-amber-500/25">
                    <CoinGlyph className="h-3.5 w-3.5 text-amber-700" />
                    {t("heroStatCoins", { n: totalCoins })}
                  </span>
                ) : null}
              </div>
            ...
            <div className="flex w-full shrink-0 justify-center lg:w-[46%] lg:justify-center xl:w-[44%]">
              <Image
                ...
                priority
                unoptimized
                className="h-auto w-52 drop-shadow-[0_12px_32px_rgba(46,10,94,0.2)] sm:w-64 lg:w-80 xl:w-96"
